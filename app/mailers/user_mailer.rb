class UserMailer < BaseMailer
  ...
  def engagement(uid)
    @user = User.find(uid)
    mail(:to => @user.email, :subject => "[ParentPins] You have a new follower") do |format|
      format.html { render :layout => 'base_mailer' } # KYLE: if you create a new layout, just put the name here
    end
  end
